...
type ReleaseLister interface {
	...
	List(selector labels.Selector) (ret []*helm.Release, err error)
...
// List lists all Releases in the indexer.
func (s *releaseLister) List(selector labels.Selector) (ret []*helm.Release, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*helm.Release))
	})
	return ret, err
}
...
type ReleaseNamespaceLister interface {
	...
	Get(name string) (*helm.Release, error)
...
// List lists all Releases in the indexer for a given namespace.
func (s releaseNamespaceLister) List(selector labels.Selector) (ret []*helm.Release, err error) {
...
// Get retrieves the Release from the indexer for a given namespace and name.
func (s releaseNamespaceLister) Get(name string) (*helm.Release, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(helm.Resource("release"), name)
	}
	return obj.(*helm.Release), nil
}
